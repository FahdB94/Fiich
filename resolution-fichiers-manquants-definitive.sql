-- =====================================================
-- RÉSOLUTION DES FICHIERS MANQUANTS DANS LE STORAGE (VERSION DÉFINITIVE)
-- =====================================================

-- 1. FONCTION POUR VÉRIFIER L'EXISTENCE D'UN FICHIER
-- =====================================================
...
-- 2. IDENTIFICATION DES DOCUMENTS ORPHELINS
-- =====================================================

-- Lister les documents orphelins
SELECT 'DOCUMENTS ORPHELINS IDENTIFIÉS:' as info;
SELECT 
    id,
    name,
    file_path,
...
-- 3. NETTOYAGE COMPLET DES DOCUMENTS ORPHELINS ET LEURS LOGS
-- =====================================================

-- Supprimer les documents orphelins et leurs logs d'audit associés
DO $$
DECLARE
    doc_record RECORD;
    deleted_count INTEGER := 0;
    logs_deleted_count INTEGER := 0;
BEGIN
    ...
    FOR doc_record IN
        ...
        WHERE NOT check_file_exists_in_storage(file_path)
    LOOP
        -- Supprimer d'abord les logs d'audit associés à ce document
        GET DIAGNOSTICS logs_deleted_count = ROW_COUNT;
        
        -- Supprimer ensuite l'entrée de la base de données
        DELETE FROM documents WHERE id = doc_record.id;
        deleted_count := deleted_count + 1;
        
        RAISE NOTICE 'Document orphelin supprimé: % (ID: %) - Logs supprimés: %', 
            doc_record.name, doc_record.id, logs_deleted_count;
    END LOOP;
    
    RAISE NOTICE 'Total documents orphelins supprimés: %', deleted_count;
END $$;
...
-- 6. NETTOYAGE DES LOGS D'AUDIT ORPHELINS RESTANTS
-- =====================================================

-- Supprimer les entrées d'audit orphelines (document_id = NULL)
SELECT 'NETTOYAGE DES LOGS D''AUDIT ORPHELINS RESTANTS:' as info;
...
CREATE OR REPLACE FUNCTION cleanup_orphaned_documents_safe()
RETURNS TABLE(
...
BEGIN
    ...
    FOR doc_record IN
        ...
        WHERE NOT check_file_exists_in_storage(file_path)
    LOOP
        -- Supprimer d'abord les logs d'audit associés
...
-- 2. Les logs d'audit associés seront également supprimés
-- 3. Seuls les documents avec fichiers existants resteront
-- 4. L'intégrité des logs d'audit sera préservée
...
-- SELECT * FROM cleanup_orphaned_documents_safe(); 