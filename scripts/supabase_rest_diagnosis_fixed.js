...
async function diagnoseSupabase() {
  console.log('🔍 Diagnostic direct Supabase via API REST (version corrigée)...\n')
  ...
  try {
    ...
    console.log(`   Status de l'insertion: ${insertResponse.statusCode}`)
    console.log(`   Réponse complète:`, insertResponse.data)
    ...
    if (insertResponse.statusCode === 201) {
      ...
      if (insertResponse.data && Array.isArray(insertResponse.data) && insertResponse.data.length > 0) {
        const insertedInvitation = insertResponse.data[0]
        console.log('   Invitation insérée:', insertedInvitation)
        
        // Nettoyer le test
        if (insertedInvitation.id) {
          const deleteResponse = await makeSupabaseRequest(`/rest/v1/invitations?id=eq.${insertedInvitation.id}`, {
            method: 'DELETE',
            useServiceRole: true
          })
          ...
          console.log('⚠️ Pas d\'ID retourné pour le nettoyage')
        }
      } else {
        console.log('⚠️ Réponse d\'insertion inattendue:', insertResponse.data)
      }
    } else {
    ...
    console.log(`   Status avec clé anon: ${anonResponse.statusCode}`)
    console.log(`   Réponse avec clé anon:`, anonResponse.data)
