...
export function EnhancedDocumentManager({ companyId }: EnhancedDocumentManagerProps) {
  ...
  const handleDownload = async (doc: Document) => {
    try {
      ...
      // Créer l'URL signée directement avec le chemin complet
      const { data, error } = await supabase.storage
        .from('company-files')
        .createSignedUrl(fullPath, 60)
  ...
  const handleDelete = async (doc: Document) => {
    try {
      ...
      const fullFilePath = `documents/${doc.file_path}`
      
      console.log('Suppression du fichier:', fullFilePath)
  ...
  const handleToggleVisibility = async (doc: Document, isPublic: boolean) => {
    try {
      const { error } = await supabase
        .from('documents')
        .update({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Gérez et organisez tous les documents de votre entreprise
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-gray-500 text-center mb-6 max-w-md">
              Aucun document n'a encore été ajouté à cette entreprise.
            </p>
            <Dialog open={showAddModal} onOpenChange={setShowAddModal}>
              ...
              <DialogContent className="sm:max-w-[600px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Sélectionnez un fichier et ajoutez les informations nécessaires.
                  </DialogDescription>
                </DialogHeader>
      ...
      {/* Bouton d'ajout pour les cas où il y a déjà des documents */}
      {documents.length > 0 && (
