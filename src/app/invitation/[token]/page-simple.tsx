...
import { Building, User, Mail, Clock, XCircle, CheckCircle } from 'lucide-react'
...
export default function InvitationPageSimple({ params }: { params: Promise<{ token: string }> }) {
  ...
  const checkAuth = async () => {
    try {
      ...
      setUser(user)
      console.log('🔍 User check:', user ? user.email : 'null')
    } catch (err) {
  ...
  const fetchInvitation = async () => {
    ...
    try {
      ...
      console.log('🔍 Tentative de récupération de l\'invitation avec le token:', token)
      ...
      console.log('✅ Invitation récupérée:', data[0])
